...
def main():
    ...
    parser.add_argument("--output_file",
                        ...
                        default='output_batch4-b.txt',
                        type=str,
                        help="The output directory where the model checkpoints will be written.")
    parser.add_argument("--model_name",
                        default='output_batch4.bin',
                        type=str,
                        help="The output directory where the model checkpoints will be written.")
    ...
    if args.load_ft:
        if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
            ...
            for input_ids, input_mask, segment_ids, label_ids, all_doc_len, all_ques_len, all_option_len in eval_dataloader:
    ...
    if args.do_train:
        ...
        for epoch in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            # logger.info("loss = %f", tr_loss / nb_tr_steps)
            if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
                eval_examples = read_swag_examples(os.path.join(args.data_dir, args.test_file), is_training=True)
                eval_features = convert_examples_to_features(
                    eval_examples, tokenizer, args.max_seq_length, True)
            ...
            if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0) and args.mid_high:
                eval_examples = read_swag_examples(os.path.join(args.data_dir, args.test_high), is_training=True)
                eval_features = convert_examples_to_features(
                    eval_examples, tokenizer, args.max_seq_length, True)
                logger.info("***** Running high evaluation *****")
                logger.info("  Num examples = %d", len(eval_examples))
                logger.info("  Batch size = %d", args.eval_batch_size)
                all_input_ids = torch.tensor(select_field(eval_features, 'input_ids'), dtype=torch.long)
                all_input_mask = torch.tensor(select_field(eval_features, 'input_mask'), dtype=torch.long)
                ...
                for input_ids, input_mask, segment_ids, label_ids, all_doc_len, all_ques_len, all_option_len in eval_dataloader:
                    ...
                    with torch.no_grad():
                        tmp_eval_loss = model(input_ids, segment_ids, input_mask, label_ids, all_doc_len, all_ques_len,
                                              all_option_len)
                        logits = model(input_ids, segment_ids, input_mask, all_doc_len, all_ques_len, all_option_len)
                ...
                result = {'eval_loss': eval_loss,
                          'eval_high_accuracy': eval_accuracy,
                          'global_step': global_step,
                          'loss': tr_loss / nb_tr_steps}
                output_eval_file = os.path.join(args.output_dir, args.output_file)
                with open(output_eval_file, "a") as writer:
            ...
            if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0) and args.mid_high:
                eval_examples = read_swag_examples(os.path.join(args.data_dir, args.test_middle), is_training=True)
                eval_features = convert_examples_to_features(
                    eval_examples, tokenizer, args.max_seq_length, True)
                logger.info("***** Running middle evaluation *****")
                logger.info("  Num examples = %d", len(eval_examples))
                logger.info("  Batch size = %d", args.eval_batch_size)
                all_input_ids = torch.tensor(select_field(eval_features, 'input_ids'), dtype=torch.long)
                all_input_mask = torch.tensor(select_field(eval_features, 'input_mask'), dtype=torch.long)
                ...
                result = {'eval_loss': eval_loss,
                          'eval_middle_accuracy': eval_accuracy,
                          'global_step': global_step,
                          'loss': tr_loss / nb_tr_steps}
                output_eval_file = os.path.join(args.output_dir, args.output_file)
                with open(output_eval_file, "a") as writer:
