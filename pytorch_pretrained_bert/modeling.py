...
class BertForMultipleChoiceWithMatch(PreTrainedBertModel):
    ...
    def __init__(self, config, num_choices=2):
        ...
        self.match = MatchNet(config)
        self.apply(self.init_bert_weights)

    def forward(self, input_ids, token_type_ids=None, attention_mask=None, doc_len=None, ques_len=None, option_len=None, labels=None):
        ...
        output_pa = self.match([doc_seq_output, ques_seq_output, ques_len])
        output_pq = self.match([doc_seq_output, option_seq_output, option_len])

        output = torch.cat([output_pa, pq_output_pq], 2)
        output_pool, _ = output.max(1)
      
        output_pool = self.dropout(output_pool)
        match_logits = self.classifier4(output_pool)
